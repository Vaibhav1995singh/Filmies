...
def watch(unique_key):
    ...
    # Retrieve recommended movies from the database
    cursor.execute("SELECT * FROM movies WHERE genre = ? OR genre_2 = ? OR genre_3 = ? OR genre_4 = ? OR genre_5 = ? AND unique_key != ? LIMIT 5", (movie[7], movie[23], movie[24], movie[25], movie[26], unique_key,))
    ...
    return render_template('watch.html', movie=movie, likes=likes, dislikes=dislikes, encoded_url=encoded_url, share_url=share_url, current_movies=current_movies, thumbnail_url=thumbnail_url, logo_url=logo_url, drive_id=drive_id)
...
def feedback(unique_key):
    ...
    if request.method == 'POST':
        ...
        return render_template('/')  # Return a response to indicate successful form submission
...
def add_series():
    if request.method == 'POST':
        ...
        c.execute('''
            INSERT INTO series (name, description, poster_url, logo_url, genre, wood, trailer_id, for_kids, ott_platform, beta_version)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (name, description, poster_url, logo_url, genre, wood, trailer_id, for_kids, ott_platform, beta_version))
...
if __name__ == '__main__':
    app.run(host='localhost',debug=True, port=4040)
